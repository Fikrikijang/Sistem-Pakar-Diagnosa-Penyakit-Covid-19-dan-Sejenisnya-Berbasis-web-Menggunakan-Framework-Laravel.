			...
			<div class="col-12">
				...
				<div class="card">
					<div class="card-header">
					  <h3 class="card-title">Daftar Tag Berita</h3>
					</div>
					@if (session('status'))
					  ...
					  <table class="table table-bordered table-striped">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{ $hasil->name }}</td>
								<td>
									<button type="button" class="btn btn-primary btn-sm" onclick="edit('{{ route('tag.edit', $hasil->id ) }}')" data-toggle="modal" data-target="#edit"><i class="fa fa-edit"></i></button>

									<button type="button" class="btn btn-danger btn-sm" onclick="$('form[name={{ $hasil->id }}]').submit();"><i class="fa fa-trash"></i></button>

									<form name="{{ $hasil->id }}" method="post" action="{{ route('tag.destroy', $hasil->id )}}">
										{{ method_field('delete') }}
										{{ csrf_field() }}
									</form>
		  ...
		  <form method="post" action="{{ route('tag.store') }}">
			...
			<div class="col-md-12">
				<div class="form-group">
					<label>Tag :</label>
					<input type="text" name="name" class="form-control" required="required" placeholder="Masukkan tag...">
				</div>
			</div>
		  </form>
		</div>
...
<div class="modal fade" id="edit">
	<div class="modal-dialog">
	  <div class="modal-content">
		<div class="modal-header">
		  <h4 class="modal-title">Ubah Data Tag</h4>
		  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
			<span aria-hidden="true">&times;</span>
		  </button>
		</div>
